...
def vec2azel(vec):
    """
    把 3-D 向量 vec（RIS中心 -> 点）转成方位角/az、俯仰角/el
    定义：az ∈ [-90,90], el ∈ [-90,90] 度
    """
    vx, vy, vz = vec / (np.linalg.norm(vec)  + 1e-15)
    az = np.degrees(np.arctan2(vx,  vy))   # 绕 Z 旋转，vy 朝外
    el = np.degrees(np.arcsin(vz))
    return az, el
...
def reconstructed_corridor_input_params(num_interactions=2, num_diffractions=1):
    ...
    # input_data.scene_settings.sdf_threshold_coarse = 0.15
    # input_data.scene_settings.sdf_threshold_refine = 0.05
    input_data.scene_settings.num_iterations = 1000
...
if __name__ == "__main__":
    ...
    # tx_pos = [3, -5, 1.2]
    tx_pos = [-0.34, -5.69, 1.5]  # scene1
    # tx_pos = [0.135, -1.23, 1.5]  # scene3
    # tx_pos = [8.397, 0.721, 0.75]  # scene4_1
    # tx_pos = [10.992, 11.333, 1.5]  # scene4_2

    ris_normal = np.array([1, 0, 0])  # scene1
    # ris_normal = np.array([0, 1, 0])  # scene3
    # ris_normal = np.array([-1, 0, 0])  # scene4_1
    # ris_normal = np.array([-1, 0, 0])  # scene4_2
    tx_key = 'tx0'
    ...
    az_list, el_list, power_list = [], [], []

    for unit, path_datas in enumerate(all_path_info_TxRxs):
        rx_pos = receiver_points[unit]
        for path_data in path_datas:
            if path_data["LOS"] == 1:
                # LOS：整条路径就是 RIS->rx 的直连
                inter_pos = rx_pos
                dir_vec = np.array(inter_pos) - ris_center
            else:
                # NLOS：取第一次交互的位置
                if path_data["Interaction Num"] == 0:
                    continue
                inter_pos = path_data["interaction 1"]["Position"]
                dir_vec = np.array(inter_pos) - ris_center

            if np.dot(dir_vec, ris_normal) < 0:
                continue
            az, el = vec2azel(dir_vec)
            az_list.append(az)
            el_list.append(el)

            # 计算接收功率
            gain_tx_rx = calculate_tx_rx_path_power(path_data, tx_pos, rx_pos)
            # gain_tx_rx 是复振幅，功率取模方
            power_list.append(gain_tx_rx)

    az_arr = np.array(az_list)
    el_arr = np.array(el_list)
    pwr_arr = np.array(power_list)
    AZ_BIN = 90  # –90°…+88° 共 90 格
    EL_BIN = 90
    dAz = 2.  # 每格 2°
    dEl = 2.

    # 轴矢量（格子中心）
    az_edges = np.arange(-90, 90, dAz) + dAz / 2  # –89° –87 … +87
    el_edges = np.arange(-90, 90, dEl) + dEl / 2

    # 初始化
    power_grid = np.full((EL_BIN, AZ_BIN), 1e-15 + 0j)

    for az, el, p in zip(az_arr, el_arr, power_list):
        # 找到所属格子序号
        az_idx = int(np.floor((az + 90) / dAz))
        el_idx = int(np.floor((el + 90) / dEl))
        # 越界保护
        az_idx = np.clip(az_idx, 0, AZ_BIN - 1)
        el_idx = np.clip(el_idx, 0, EL_BIN - 1)
        # power_grid[el_idx, az_idx] = max(power_grid[el_idx, az_idx], p)
        power_grid[el_idx, az_idx] += p

    power_grid = 10 * np.log10(np.abs(power_grid) ** 2)
    plt.figure(figsize=(7, 5.5))
    # 用 pcolormesh 可以让格子边缘对齐坐标轴
    az_map = np.linspace(-90, 90, AZ_BIN + 1)
    el_map = np.linspace(-90, 90, EL_BIN + 1)
    plt.pcolormesh(az_map, el_map, power_grid, cmap='jet', shading='flat', vmin=-150, vmax=np.nanmax(power_grid))
    cb = plt.colorbar(label='Received  power [dB]')
    cb.set_label('Received  power [dB]', fontsize=10)
    plt.xlabel('Azimuth  angle [deg]')
    plt.ylabel('Elevation  angle [deg]')
    plt.title('RIS  radiation pattern (2°×2° grid)')
    plt.xlim([-90, 90])
    plt.ylim([-90, 90])
    ...
    binary_grid = (power_grid > -120).astype(np.uint8)
    struct = np.ones((3, 3), dtype=np.uint8)  # 8-连通结构元
    binary_grid_dilated = binary_dilation(binary_grid, structure=struct).astype(np.uint8)
    plt.figure(figsize=(7, 5.5))
    # 用灰度色盘：0 画成黑，1 画成白
    plt.pcolormesh(az_map, el_map, binary_grid_dilated, cmap='jet', shading='flat', vmin=0, vmax=1)
    cb = plt.colorbar(ticks=[0, 1])
    plt.xlabel('Azimuth  angle [deg]')
    plt.ylabel('Elevation  angle [deg]')
    plt.title('Binary  mask: power > −120 dB')
    plt.xlim([-90, 90])
    plt.ylim([-90, 90])
    # plt.grid(True, which='both', linestyle=':', linewidth=0.3)
    plt.tight_layout()
    ...
    savemat(f'data_all/coding/{mat_name}.mat', {mat_name: binary_grid_dilated})
    print(f'已写入 {mat_name}.mat')
