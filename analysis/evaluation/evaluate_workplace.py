...
BASE_DIR = REPO_ROOT / 'data' / 'workplace'
BER_DIR = os.path.join(BASE_DIR, 'BER_data')
POWER_DIR = os.path.join(BASE_DIR, 'Power_data')

# region_colors = ['#f0f9e8', '#ccebc5', '#a8ddb5', '#7bccc4', '#4eb3d3']
region_colors = ['#ffffff', '#ffffff', '#ffffff', '#ffffff', '#ffffff']
...
def plot_ber(ax, data_no, data_with, ylabel):
    ...
    for i in range(num_areas):
        ...
        # ax.text(mid_x, y_text_pos, f'Area {i + 1}', ha='center', va='bottom', fontsize=fontsize, fontweight='bold')

    ax.grid(True, which='major', linestyle='--', linewidth=0.5, alpha=0.5)
    # ax.set_xlabel('Index of points')
    # ax.set_ylabel(ylabel)
    ax.set_xlim(0.5, total_points + 0.5)
    ax.set_xticks(np.arange(0, total_points + 1, 12))
