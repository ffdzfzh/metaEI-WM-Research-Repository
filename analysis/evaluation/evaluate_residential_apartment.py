...
BASE_DIR = REPO_ROOT / 'data' / 'residential_apartment'
...
def load_series_data(folder_path, col_name=None, is_power1=False, is_power2=False):
...
def plot_ber(ax, data_no, data_with, ylabel):
    ...
    min_len = min(len(data_no), len(data_with), total_points)
    y1 = np.array(data_no[:min_len])
    y2 = np.array(data_with[:min_len])
    x = x[:min_len]
    ...
    y_lims = ax.get_ylim()
    y_top = y_lims[1]
    y_text_pos = y_top - (y_top - y_lims[0]) * 0.05

    for i in range(num_areas):
        start_x = i * points_per_area
        end_x = (i + 1) * points_per_area
        if i < num_areas - 1:
            ax.axvspan(start_x, end_x, facecolor=region_colors[i], alpha=0.2)
        else:
            ax.axvspan(start_x, end_x + 1, facecolor=region_colors[i], alpha=0.2)
        # mid_x = (start_x + end_x) / 2
        # ax.text(mid_x, y_text_pos, f'Area {i + 1}', ha='center', fontsize=12, fontweight='bold')

    ax.grid(True, which='major', linestyle='--', linewidth=0.5, alpha=0.5)
    # ax.set_xlabel('Index of points')
    # ax.set_ylabel(ylabel)
    ax.set_xlim(0.5, total_points + 0.5)
    ax.set_xticks(np.arange(0, total_points + 1, 5))
...
def plot_power_comparison1(ax, meas, sim, ylabel):
    ...
    min_len = min(len(meas), len(sim), total_points)
    y_meas, y_sim, x = np.array(meas[:min_len]), np.array(sim[:min_len]), x[:min_len]
...
def plot_bar_improvement(ax, meas_diff, sim_diff):
    ...
    min_len = min(len(meas_diff), len(sim_diff), total_points)
    y_meas, y_sim, x = np.array(meas_diff[:min_len]), np.array(sim_diff[:min_len]), x[:min_len]
    ...
    for i in range(num_areas):
        start_x = i * points_per_area + 0.5
        end_x = (i + 1) * points_per_area + 0.5
        # if i < num_areas - 1:
        #     ax.axvspan(start_x - 0.5, end_x - 0.5, facecolor=region_colors[i], alpha=0.2)
        # else:
        #     ax.axvspan(start_x - 0.5, end_x + 1, facecolor=region_colors[i], alpha=0.2)
...
def main():
    ...
    pow_no = load_series_data(os.path.join(POWER_DIR, 'no_ris'), is_power2=True)
    pow_with = load_series_data(os.path.join(POWER_DIR, 'with_ris'), is_power2=True)
    pow_no_sim = load_series_data(os.path.join(POWER_DIR, 'no_ris_sim'), is_power1=True)
    pow_with_sim = load_series_data(os.path.join(POWER_DIR, 'with_ris_sim'), is_power1=True)
    ...
    r_no = np.corrcoef(pow_no, pow_no_sim)[0, 1]
    d_no = willmott_index(pow_no, pow_no_sim)
    r_with = np.corrcoef(pow_with, pow_with_sim)[0, 1]
    ...
    for i in range(1):
        ...
        if start < len(pow_meas_diff):
            ...
            print(f"Area {i + 1}: measured mean gain = {meas_mean:5.2f} dB  |  simulated mean gain = {sim_mean:5.2f} dB")
